...
it("can delete account with verified emails", () =>
  withUserDb(async (client, user) => {
    await client.query("select * from dum_public.request_account_deletion()");
    ...
    const { rows } = await client.query(
      "select dum_public.confirm_account_deletion($1) as deleted",
      [token]
    );
    ...
    } = await client.query("select dum_public.current_user_id()");
