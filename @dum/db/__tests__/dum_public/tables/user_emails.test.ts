...
async function addEmail(client: PoolClient, email: string, verified = false) {
  const {
    ...
    `insert into dum_public.user_emails (email) values ($1) returning *`,
    [email]
  );
  if (verified) {
    await asRoot(client, () =>
      client.query(
        "update dum_public.user_emails set is_verified = true where id = $1",
        [row.id]
      )
    );
  }
...
it("cannot manually create a verified email", () =>
  withUserDb(async (client) => {
    const promise = client.query(
      `insert into dum_public.user_emails (email, is_verified) values ($1, true) returning *`,
...
it("can promote a verified email to primary over another verified email", () =>
  withUserDb(async (client) => {
    ...
    const {
      rows: [email2],
    } = await client.query("select * from dum_public.make_email_primary($1)", [
      email.id,
    ]);
...
it("cannot promote a non-verified email to primary", () =>
  withUserDb(async (client) => {
    ...
    expect(email.is_primary).toBe(false);
    const promise = client.query(
      "select * from dum_public.make_email_primary($1)",
      [email.id]
    );
  ...
  withRootDb(async (client) => {
    ...
    // Block-scoped variables FTW
    {
      const { rows } = await client.query(
        "select * from dum_public.user_emails order by id asc"
      );
      const userIds = rows
        .map((row) => row.user_id)
...
it("cannot delete last verified email", () =>
  withRootDb(async (client) => {
    ...
    const promise = client.query(
      "delete from dum_public.user_emails where user_id = $1 and id <> $2",
      [user.id, email4.id]
    );
...
it("cannot delete last unverified email if no verified emails", () =>
  withRootDb(async (client) => {
    ...
    const promise = client.query(
      "delete from dum_public.user_emails where user_id = $1",
      [user.id]
    );
    await expect(promise).rejects.toMatchObject({
      code: "CDLEA",
...
it("can delete all but one verified email", () =>
  withRootDb(async (client) => {
    ...
    const { rows } = await client.query(
      "select * from dum_public.user_emails where user_id = $1",
      [user.id]
    );
