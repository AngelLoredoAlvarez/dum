 ...
 * with this function.
 */
create function dum_public.resend_email_verification_code(email_id uuid) returns boolean as $$
begin
  if exists(
    select 1
    from dum_public.user_emails
    where user_emails.id = email_id
    and user_id = dum_public.current_user_id()
...
comment on function dum_public.resend_email_verification_code(email_id uuid) is
