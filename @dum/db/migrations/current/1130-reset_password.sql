 ...
 * `dum_public.forgot_password` for the first half.
 *
 * The `dum_private.reset_password` function checks the reset token is correct
 ...
 * function into dum_private and explicitly call it from the `resetPassword`
 * field in `PassportLoginPlugin.ts`.
 */

create function dum_private.assert_valid_password(new_password text) returns void as $$
begin
...
create function dum_private.reset_password(user_id uuid, reset_token text, new_password text) returns boolean as $$
declare
