 ...
 *
 * `dum_public.forgot_password` is responsible for checking these things and
 * queueing a reset password token to be emailed to the user. For what happens
 * after the user receives this email, see instead `dum_private.reset_password`.
 *
 * NOTE: unlike dum_private.login and dum_private.reset_password, rolling back
...
create table dum_private.unregistered_email_password_resets (
  email citext constraint unregistered_email_pkey primary key,
  attempts int not null default 1,
  latest_attempt timestamptz not null
);
comment on table dum_private.unregistered_email_password_resets is
...
comment on column dum_private.unregistered_email_password_resets.attempts is
  E'We store the number of attempts to help us detect accounts being attacked.';
comment on column dum_private.unregistered_email_password_resets.latest_attempt is
  E'We store the time the last password reset was sent to this email to prevent the email getting flooded.';

/**********/

create function dum_public.forgot_password(email citext) returns void as $$
declare
...
begin
  ...
  if v_user_email is null then
    ...
    insert into dum_private.unregistered_email_password_resets (email, latest_attempt)
  ...
  update dum_private.user_email_secrets
...
comment on function dum_public.forgot_password(email public.citext) is
