 ...
 */
create table dum_public.user_emails (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null default dum_public.current_user_id() references dum_public.users on delete cascade,
...
alter table dum_public.user_emails enable row level security;
...
create unique index uniq_user_emails_verified_email on dum_public.user_emails(email) where (is_verified is true);
-- Only one primary email per user.
create unique index uniq_user_emails_primary_email on dum_public.user_emails (user_id) where (is_primary is true);
-- Allow efficient retrieval of all the emails owned by a particular user.
create index idx_user_emails_user on dum_public.user_emails (user_id);
-- For the user settings page sorting
create index idx_user_emails_primary on dum_public.user_emails (is_primary, user_id);

-- Keep created_at and updated_at up to date.
create trigger _100_timestamps
  before insert or update on dum_public.user_emails
  for each row
...
create trigger _500_audit_added
  after insert on dum_public.user_emails
  for each row
...
create trigger _500_audit_removed
  after delete on dum_public.user_emails
  for each row
  execute procedure dum_private.tg__add_audit_job(
    'removed_email',
    'user_id',
...
create function dum_public.tg_user_emails__forbid_if_verified() returns trigger as $$
begin
  if exists(select 1 from dum_public.user_emails where email = NEW.email and is_verified is true) then
    raise exception 'An account using that email address has already been created.' using errcode='EMTKN';
  end if;
  return NEW;
end;
$$ language plpgsql volatile security definer set search_path to pg_catalog, public, pg_temp;
create trigger _200_forbid_existing_email before insert on dum_public.user_emails for each row execute procedure dum_public.tg_user_emails__forbid_if_verified();
...
create trigger _900_send_verification_email
  ...
  execute procedure dum_private.tg__add_job('user_emails__send_verification');

comment on table dum_public.user_emails is
  E'Information about a user''s email address.';
comment on column dum_public.user_emails.email is
  E'The users email address, in `a@b.c` format.';
comment on column dum_public.user_emails.is_verified is
...
create policy select_own on dum_public.user_emails for select using (user_id = dum_public.current_user_id());
create policy insert_own on dum_public.user_emails for insert with check (user_id = dum_public.current_user_id());
-- NOTE: we don't allow emails to be updated, instead add a new email and delete the old one.
create policy delete_own on dum_public.user_emails for delete using (user_id = dum_public.current_user_id());

grant select on dum_public.user_emails to :DATABASE_VISITOR;
grant insert (email) on dum_public.user_emails to :DATABASE_VISITOR;
-- No update
...
create function dum_public.tg_user_emails__prevent_delete_last_email() returns trigger as $$
begin
  if exists (
    with remaining as (
...
create trigger _500_prevent_delete_last
  ...
  for each statement
  execute procedure dum_public.tg_user_emails__prevent_delete_last_email();
 ...
 */
create table dum_private.user_email_secrets (
  user_email_id uuid primary key references dum_public.user_emails on delete cascade,
  verification_token text,
  verification_email_sent_at timestamptz,
  password_reset_email_sent_at timestamptz
);
alter table dum_private.user_email_secrets enable row level security;

comment on table dum_private.user_email_secrets is
...
comment on column dum_private.user_email_secrets.password_reset_email_sent_at is
  E'We store the time the last password reset was sent to this email to prevent the email getting flooded.';

create function dum_private.tg_user_email_secrets__insert_with_user_email() returns trigger as $$
declare
...
begin
  ...
  insert into dum_private.user_email_secrets(user_email_id, verification_token) values(NEW.id, v_verification_token);
  return NEW;
end;
$$ language plpgsql volatile security definer set search_path to pg_catalog, public, pg_temp;
create trigger _500_insert_secrets
  after insert on dum_public.user_emails
  for each row
  execute procedure dum_private.tg_user_email_secrets__insert_with_user_email();
comment on function dum_private.tg_user_email_secrets__insert_with_user_email() is
 ...
 */
create function dum_public.verify_email(user_email_id uuid, token text) returns boolean as $$
begin
  update dum_public.user_emails
  set
    is_verified = true,
    is_primary = is_primary or not exists(
      select 1 from dum_public.user_emails other_email where other_email.user_id = user_emails.user_id and other_email.is_primary is true
    )
  where id = user_email_id
  and exists(
    select 1 from dum_private.user_email_secrets where user_email_secrets.user_email_id = user_emails.id and verification_token = token
  );
  return found;
end;
$$ language plpgsql strict volatile security definer set search_path to pg_catalog, public, pg_temp;
comment on function dum_public.verify_email(user_email_id uuid, token text) is
...
create function dum_public.tg_user_emails__verify_account_on_verified() returns trigger as $$
begin
  update dum_public.users set is_verified = true where id = new.user_id and is_verified is false;
  return new;
end;
$$ language plpgsql strict volatile security definer set search_path to pg_catalog, public, pg_temp;
...
create trigger _500_verify_account_on_verified
  ...
  on dum_public.user_emails
  for each row
  when (new.is_verified is true)
  execute procedure dum_public.tg_user_emails__verify_account_on_verified();
