...
-- key columns.
create function dum_private.tg__add_job() returns trigger as $$
begin
  perform graphile_worker.add_job(tg_argv[0], json_build_object('id', NEW.id));
  return NEW;
end;
$$ language plpgsql volatile security definer set search_path to pg_catalog, public, pg_temp;
comment on function dum_private.tg__add_job() is
...
create function dum_private.tg__add_audit_job() returns trigger as $$
declare
  v_user_id uuid;
...
$$ language plpgsql volatile security definer set search_path to pg_catalog, public, pg_temp;
comment on function dum_private.tg__add_audit_job() is
 ...
 */
create function dum_private.tg__timestamps() returns trigger as $$
begin
...
comment on function dum_private.tg__timestamps() is
 ...
 *       );
 */
create function dum_public.tg__graphql_subscription() returns trigger as $$
declare
...
comment on function dum_public.tg__graphql_subscription() is
