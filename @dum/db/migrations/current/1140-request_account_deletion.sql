 ...
 * `dum_public.confirm_account_deletion` for the second half.
 */

create function dum_public.request_account_deletion() returns boolean as $$
declare
...
begin
  ...
  select * into v_user_email
    ...
    where user_id = dum_public.current_user_id()
...
comment on function dum_public.request_account_deletion() is
