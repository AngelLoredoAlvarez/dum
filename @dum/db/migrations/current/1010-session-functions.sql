...
-- Note we have this in `dum_public` but it doesn't show up in the GraphQL
-- schema because we've used `postgraphile.tags.jsonc` to omit it. We could
-- have put it in dum_hidden to get the same effect more easily, but it's often
-- useful to un-omit it to ease debugging auth issues.
create function dum_public.current_session_id() returns uuid as $$
  select nullif(pg_catalog.current_setting('jwt.claims.session_id', true), '')::uuid;
$$ language sql stable;
comment on function dum_public.current_session_id() is
 ...
 */
create function dum_public.current_user_id() returns uuid as $$
  select user_id from dum_private.sessions where uuid = dum_public.current_session_id();
$$ language sql stable security definer set search_path to pg_catalog, public, pg_temp;
comment on function dum_public.current_user_id() is
