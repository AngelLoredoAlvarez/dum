...
const currentUserTopicFromContext = async (
  ...
  if (context.sessionId /* fail fast */) {
    ...
    const {
      ...
      "select dum_public.current_user_id() as id"
    );
    if (user) {
 ...
 *
 * And see the database trigger function `dum_public.tg__graphql_subscription()`.
 */
  ...
  return {
    ...
    resolvers: {
      UserSubscriptionPayload: {
        user: recordByIdFromTable(build, sql.fragment`dum_public.users`),
      },
    },
  };
});
