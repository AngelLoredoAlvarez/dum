import { projectName } from "@dum/config";
...
const task: Task = async (rawPayload, { addJob, withPgClient, job }) => {
  ...
  const {
    ...
    client.query<{
      id: string;
      created_at: Date;
    }>("select * from dum_public.users where id = $1", [payload.user_id])
  );
  ...
  const { rows: userEmails } = await withPgClient((client) =>
    client.query<{
      ...
      "select * from dum_public.user_emails where user_id = $1 and is_verified is true order by id asc",
      [payload.user_id]
    )
  );
