...
const task: Task = async (inPayload, { addJob, withPgClient }) => {
  ...
  const {
    ...
    pgClient.query(
      `
        select user_emails.id, email, verification_token, name, extract(epoch from now()) - extract(epoch from verification_email_sent_at) as seconds_since_verification_sent
        from dum_public.user_emails
        inner join dum_private.user_email_secrets
        on user_email_secrets.user_email_id = user_emails.id
        inner join dum_public.users
  ...
  const { email, verification_token, name, seconds_since_verification_sent } =
    userEmail;
  if (
  ...
  await withPgClient((pgClient) =>
    pgClient.query(
      "update dum_private.user_email_secrets set verification_email_sent_at = now() where user_email_id = $1",
      [userEmailId]
    )
  );
};
