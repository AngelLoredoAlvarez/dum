...
# Set to 1 only if you're on Node v12 of higher; enables advanced optimisations:`
  ...
  add("REDIS_URL", answers.DATABASE_NAME, "# REDIS url for the REDIS server");
...
exports.checkGit = async function checkGit() {
  try {
    ...
    console.error();
    console.error(
      "ERROR: The project must run inside of a git versioned folder. Please run the following:"
    );
    console.error();
    console.error("  git init");
    console.error("  git add .");
    console.error("  git commit -m 'First commit'");
    console.error();
    process.exit(1);
  }
};
